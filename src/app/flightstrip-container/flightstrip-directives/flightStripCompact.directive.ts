...
export class FlightStripCompact implements OnInit, OnDestroy {
  ...
  updateStyle() {
    switch (this.fontSize) {
      case "small":
        this.elementRef.nativeElement.style.fontSize = `${7 * this.styleChanger.multiplier}pt`
        break;
      case "medium":
        this.elementRef.nativeElement.style.fontSize = `${10 * this.styleChanger.multiplier}pt`
        break;
      case "large":
        this.elementRef.nativeElement.style.fontSize = `${15 * this.styleChanger.multiplier}pt`
        break;
    }
  }
